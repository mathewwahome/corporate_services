...
const LL_METHOD =
	"corporate_services.icl_corporate_services.doctype.project_management_lessons_learned.project_management_lessons_learned.get_active_template_questions";

frappe.ui.form.on("Project Management Lessons Learned", {
	onload(frm) {
	...
	frappe.call({
		...
		callback: (r) => {
			...
			if (!payload.template_name) {
				frappe.msgprint(__("No active Lessons Learned Template found. Please contact HR/System Admin."));
				return;
			}
...
function populate_questions_from_template(frm) {
	...
	frappe.call({
		method: LL_METHOD,
		args: { template_name: frm.doc.question_template || null },
		callback: (r) => {
			const payload = r.message || {};
...
function render_questionnaire_ui(frm) {
	...
	frm._ll_response_controls = [];

	const rows = frm.doc.answers || [];
	if (!rows.length) {
		field.$wrapper.html(
			'<div class="text-muted">No active lessons learned questions are available right now.</div>',
		);
		return;
	}
			...
			return `
				<div class="card border mb-3">
					<div class="card-body">
						...
						<div class="ll-question-response-control" id="${inputId}" data-row-idx="${idx}"></div>
					</div>
				</div>
			`;
		})
	...
	field.$wrapper.html(`<div class="ll-questionnaire-ui">${cards}</div>`);
	field.$wrapper.find(".ll-question-response-control").each(function () {
		...
		frm._ll_response_controls.push({ idx, control });
		const saveValue = () => {
...
function sync_questionnaire_ui_to_rows(frm) {
	const controls = frm._ll_response_controls || [];
