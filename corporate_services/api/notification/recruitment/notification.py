...
@frappe.whitelist()
def send_application_received_email(doc_or_docname, method=None):
    """
    Sends an auto-acknowledgment email when a job application is received.
    """
    ...
    if doc.custom_application_stage != "APPLICATION RECEIVED":
        return

    if not doc.is_new() and not doc.has_value_changed("custom_application_stage"):
        return
    ...
    role_title = job_opening.custom_role if job_opening else doc.custom_role
