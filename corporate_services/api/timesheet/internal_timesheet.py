...
@frappe.whitelist()
def internal_single_timesheet(docname):
    try:
        ...
        if not file_content:
            frappe.log_error(f"No content retrieved from file: {file_url}", "internal_single_timesheet")
            return "error"
        ...
        timesheet = frappe.new_doc("Internal Timesheet")
        timesheet.employee = doc.employee
        timesheet.month = doc.month
        timesheet.insert()
        ...
        for row in reader:
            ...
            if len(row) > 0 and row[0] and row[0] not in ['Projects', 'Meetings', 'Proposals', 'Recurring Tasks', 'Other Tasks/Activities', 'Training/Workshops/Connectathons']:
                ...
                if current_project in existing_projects:
                    try:
                        frappe.logger().info(f"Started timesheet for project: {current_project}", "timesheet_import")
                    except Exception as e:
                        frappe.log_error(f"Error handling project {current_project}: {str(e)}", "timesheet_import")
                        current_project = None
                else:
            ...
            if current_project and len(row) > 1:
                ...
                frappe.log_error(f"project_id: {project_id}", "timesheet_import")
                ...
                for idx in range(2, len(header)):
                    ...
                    if row[idx]:
                        try:
                            ...
                            try:
                                timesheet_detail = frappe.new_doc("Internal Timesheet Details")
        ...
        frappe.log_error(frappe.get_traceback(), "internal_single_timesheet")
        return "error"
...
def is_time_overlap(employee, from_time, to_time):
    ...
    for submission in submissions:
        details = frappe.get_all("Internal Timesheet Details",
